...
export default function ReadingPlanPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      <div>
        <h1 className="font-serif text-2xl font-bold text-scripture-800 mb-1">Reading Plan</h1>
        <p className="text-sm text-gray-500">Follow a structured daily reading plan through Scripture.</p>
      </div>
          ...
          <div key={plan.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {plan.days.map(dayEntry => (
                <div
                  key={dayEntry.day}
                  className={`flex items-center gap-3 px-4 py-3 ${dayEntry.completed ? 'bg-green-50' : 'hover:bg-gray-50'}`}
                >
                  <button
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className={`text-sm font-medium truncate ${dayEntry.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                        {dayEntry.title}
                      </span>
                    </div>
                    <button
                      onClick={() => navigate(`/reader?book=${dayEntry.bookId}&chapter=${dayEntry.chapter}`)}
                      className="text-xs text-scripture-600 hover:text-scripture-800 mt-0.5"
                    >
                      {dayEntry.reference}
                    </button>
                  </div>
      ...
      {plans.length === 0 && (
        <div className="text-center py-14 text-gray-400">
          ...
          <p className="font-medium text-gray-500">No reading plans available</p>
        </div>
      )}
    </div>
  );
