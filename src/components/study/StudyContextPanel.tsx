...
export default function StudyContextPanel({ verse, onClose }: StudyContextPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-scripture-50 dark:bg-gray-700">
        <div>
          <p className="text-xs text-scripture-500 dark:text-scripture-300 font-medium uppercase tracking-wide">Study Context</p>
          <h3 className="font-serif font-semibold text-scripture-800 dark:text-scripture-100">{verseRef}</h3>
        </div>
        <button onClick={onClose} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-xl leading-none px-1" aria-label="Close panel">✕</button>
      </div>

      {/* Verse text */}
      <div className="px-4 py-3 bg-parchment-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
        <p className="font-serif text-gray-800 dark:text-gray-100 leading-relaxed italic">"{verse.text}"</p>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{verseRef}</p>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex-wrap">
        <button
          ...
          className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
            fav ? 'bg-red-100 text-red-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600'
          }`}
        >
          {fav ? '♥' : '♡'} {fav ? 'Saved' : 'Favorite'}
        </button>
        <div className="flex items-center gap-1">
          <span className="text-xs text-gray-500 dark:text-gray-400 mr-1">Highlight:</span>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        ...
        {ctx.bookContext && (
          <>
            ...
            <StudySection title="Genre & Literary Style" defaultOpen={false}>
              <p className="font-medium text-gray-700 dark:text-gray-300 mb-1">{ctx.bookContext.genre}</p>
        ...
        <StudySection title="My Notes">
          <textarea
            ...
            className="w-full min-h-[100px] text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded p-2 focus:outline-none focus:ring-2 focus:ring-scripture-400 resize-y placeholder-gray-400 dark:placeholder-gray-500"
          />
